...
// -----------------------------------------------------------------------------
// Description:
//   Wrapper around two GF180 128x8 SRAM macros to form one 128x16 memory.
//
...
//   - 128 halfwords
//   - 256 bytes
//   - 64 RV32 instructions when used as instruction memory
//
// Boot mode:
//   - Writes one byte at a time using boot_byte_addr[7:0].
//   - boot_byte_addr[0] selects low/high byte.
//   - boot_byte_addr[7:1] selects halfword address.
//
// Run mode:
...
module sram128x16 (
    ...
    input  wire        boot_we,
    input  wire [7:0]  boot_byte_addr,
    input  wire [7:0]  boot_wdata,

    // Run/read interface
    input  wire [6:0]  run_addr,
    ...
    wire [6:0] boot_half_addr;
    wire       boot_byte_sel;

    assign boot_half_addr = boot_byte_addr[7:1];
    ...
    gf180mcu_fd_ip_sram__sram128x8m8wm1 u_mem_b0 (
        .CLK  (clk),
        .CEN  (cen),
        .GWEN (gwen_b0),
        .WEN  (wen_b0),
    ...
    gf180mcu_fd_ip_sram__sram128x8m8wm1 u_mem_b1 (
        .CLK  (clk),
        .CEN  (cen),
        .GWEN (gwen_b1),
        .WEN  (wen_b1),
